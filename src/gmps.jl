...
function hybrid_eleck_slater_to_mps(
    s::Vector{<:Index},
    Φ_up::AbstractMatrix,
    Φ_dn::AbstractMatrix,
    modulus_ky::Int;
    ...
    N = length(s)
    # hybrid space method for slater to MPS
    # the code is generated by gemini-3
    # correlation function Λ = Φ * Φ'
    Λ_up = Φ_up * Φ_up'
    Λ_dn = Φ_dn * Φ_dn'
    MPS_Elt = promote_type(eltype(Λ_up), eltype(Λ_dn))

    # 1. momentum sectors
    ky_blocks = [Int[] for _ in 1:modulus_ky]
    for n in 1:N
        ky = (n - 1) % modulus_ky
        push!(ky_blocks[ky + 1], n)
    end

    total_rotations = Givens{MPS_Elt}[]
    ns_total = zeros(real(MPS_Elt), 2 * N)

    # 2. Givens rotation for different sector
    for block in ky_blocks
        isempty(block) && continue
        ns_up, C_up = correlation_matrix_to_gmps(ConservesNf(Λ_up[block, block]); eigval_cutoff=eigval_cutoff)
        ns_dn, C_dn = correlation_matrix_to_gmps(ConservesNf(Λ_dn[block, block]); eigval_cutoff=eigval_cutoff)

        for (i, n) in enumerate(block)
            ns_total[2n - 1] = ns_up[i]
            ns_total[2n]     = ns_dn[i]
        end
        for g in C_up.rotations
            push!(total_rotations, Givens(2 * block[g.i1] - 1, 2 * block[g.i2] - 1, g.c, g.s))
        end
        for g in C_dn.rotations
            push!(total_rotations, Givens(2 * block[g.i1], 2 * block[g.i2], g.c, g.s))
        end
    end

    # 3. fermion index ( QN name the same as ElecK)
    sf = Index{Vector{Pair{QN, Int}}}[]
    for n in 1:N
        mod_ky = (n - 1) % modulus_ky
        # match the def of ElecK
        push!(sf, Index(QN(("Nf",0,-1),("Sz",0),("Ky",0,modulus_ky))=>1, 
                        QN(("Nf",1,-1),("Sz",1),("Ky",mod_ky,modulus_ky))=>1; tags="Fermion,Up,n=$n"))
        push!(sf, Index(QN(("Nf",0,-1),("Sz",0),("Ky",0,modulus_ky))=>1, 
                        QN(("Nf",1,-1),("Sz",-1),("Ky",mod_ky,modulus_ky))=>1; tags="Fermion,Dn,n=$n"))
    end

    # 4.  prepare 2N fermionic MPS and apply rotation gate
    C_total = ConservesNf(Circuit(total_rotations))
    U = itensors(sf, C_total)
    psi_f = MPS(MPS_Elt, sf, i -> round(Int, ns_total[i]) + 1)
    
    # clean the kwargs
    itensor_kwargs = (:maxdim, :cutoff, :mindim, :normalize)
    clean_kwargs = Dict(k => v for (k, v) in kwargs if k in itensor_kwargs)
    psi_f = apply(U, psi_f; clean_kwargs...)

    # 5. manually map 2N fermion to N-elecK sites
    psi = MPS(N)
    for n in 1:N
        # construct mapping tensor M: [sf_up, sf_dn, dag(s_eleck)]
        # avoid combiner
        # M = ITensor(sf[2n-1], sf[2n], dag(s[n]))
        M = ITensor(dag(sf[2n-1]), dag(sf[2n]), s[n])
        M[dag(sf[2n-1])=>1, dag(sf[2n])=>1, s[n]=>1] = 1.0 # Empty
        M[dag(sf[2n-1])=>2, dag(sf[2n])=>1, s[n]=>2] = 1.0 # Up
        M[dag(sf[2n-1])=>1, dag(sf[2n])=>2, s[n]=>3] = 1.0 # Down
        M[dag(sf[2n-1])=>2, dag(sf[2n])=>2, s[n]=>4] = 1.0 # Double
        
        # contract two pseudo-site to physical site
        T = (psi_f[2n-1] * psi_f[2n]) * M
        psi[n] = T
    end
    # check norm in case of zero norm psi
    println(norm(psi))
    orthogonalize!(psi, 1)
    return psi
end
...
function correlation_matrix_to_mps(
  ...
  elseif all(hastags("ElecK"), s)
    modulus_ky = kwargs[:Ny]
    return hybrid_eleck_slater_to_mps(
      s, Λ_up, Λ_dn, modulus_ky; eigval_cutoff=eigval_cutoff, kwargs...
    )
  end
end
